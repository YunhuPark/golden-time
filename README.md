# 🚑 Golden-Time - 실시간 응급실 검색 및 최적 이송 매칭 시스템

> **응급 상황 발생 시 가장 빠르고 확실한 병상 확보**
> 전국의 응급실 병상 가용 현황을 실시간으로 파악하여 골든타임을 지켜냅니다.
...
**Golden-Time**은 응급 상황에서 환자를 수용 가능한 최적의 병원으로 안내하는 **실시간 응급실 매칭 시스템**입니다.
이 프로젝트는 독자적인 애플리케이션으로 동작하며, 외부 AI 의료 영상 분석 프로젝트인 **Medi-Matrix** 시스템에서 위급 환자로 판별될 경우 자동으로 호출되어 환자의 안전한 이송을 책임집니다.

---

## ✨ 핵심 기능

### 🚨 실시간 응급실 검색 & 스마트 추천 매칭
- **병상 가용률 실시간 반영**: 국립중앙의료원(E-Gen) API를 호출하여 현재 가용 병상이 있는 병원을 최우선으로 안내합니다.
- **AI + 거리 + 병상 복합 스코어링**: 최단 거리(40%), 병상 여유(35%), 환자 상태 기반 특화도(25%)를 복합적으로 스코어링하여 최적의 병원을 추천합니다.
- **만실(0병상) 긴급 경고**: 반경 내 응급실이 모두 만실일 경우, 레드 알럿(119 즉시 호출) UI로 자동 전환되어 생명을 보호합니다.

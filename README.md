...
- **경로 소요시간 (최대 40점)**: 가장 빠른 병원 40점, 가장 느린 병원 0점 비례 할당. 경로 정보가 없으면 0점.
- **병상 가용률 (최대 30점)**: 가용 비율에 따라 20~30점 부여. 제한적일 시 15점, 만실이면 0점 처리.
- **질환 적합도 (최대 30점)**: AI 기반 질병-진료과 매칭 점수 합산. (DB 데이터의 신뢰도 기반)
- **외상센터 등급 (최대 20점)**: 권역외상센터 20점, 지역외상센터 15점, 일반센터 10점, 없음 5점.
- **응급실 운영 여부 (최대 10점)**: 현재 운영 중일 시 10점.
- **구급차 가용 역량 (최대 10점)**: 중환자실(ICU), 응급수술 가용 등 역량 지표 합산 점수.

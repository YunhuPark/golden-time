# 🏥 Medi-Matrix (구 Golden Time) - AI 기반 응급실 자동 매칭 시스템

> **생명이 걸린 순간, AI가 가장 빠르고 정확한 병원을 찾아드립니다**
> 실시간 응급실 병상 현황 + AI 질환 특화 병원 추천 + GPS 기반 최적 경로 안내
...
**Medi-Matrix**는 응급 상황에서 사용자의 현재 위치를 기반으로 주변 응급실의 **실시간 병상 현황**을 조회하고, **AI(LLM) 크롤러**가 수집한 병원별 특화 분야(중증 외상, 심뇌혈관 등) 데이터를 결합하여 환자의 증상에 맞는 가장 완벽한 병원을 추천해주는 지능형 웹 애플리케이션입니다.

### ✨ 핵심 기능

#### 🤖 AI 병원 특화 분야 크롤링 파이프라인 (자동화)
...
- **AI + 거리 + 병상 복합 스코어링**: 거리가 가깝고(30%), 병상이 넉넉하며(30%), 환자의 증상과 병원의 특화 분야가 일치하는(40%) 최적의 병원을 1순위로 추천합니다.

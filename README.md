...
### 🤖 AI 기반 병원 특화 분야 데이터 파이프라인 및 매칭
- **질환 적합도 기반 추천**: AI가 직접 진단하는 것이 아니라, 수집된 공개 자료와 휴리스틱을 활용해 질환 적합도를 추천 점수에 반영합니다.
- **데이터 파이프라인 흐름**: Naver 뉴스/블로그 검색 API를 기반으로 병원 관련 텍스트 데이터를 수집하고, OpenAI LLM 모델을 통해 특화 분야(질환) 키워드를 추출하여 병원과 매칭합니다.
- **휴리스틱 Fallback**: 수집된 데이터가 없거나 OpenAI API 키가 설정되지 않은 경우, 병원명 및 권역외상센터 등급에 기반한 자체 휴리스틱 알고리즘을 사용합니다.
- **비용 최적화 캐싱**: 빈번한 API 호출을 줄이기 위해 데이터는 30일(TTL) 단위로만 재갱신되며, 유효한 데이터가 수집되지 않은 병원도 빈 상태로 캐싱하는 Negative Cache가 구현되어 있습니다.
...
2. 다음 API 신청:
   ...
   - **응급의료기관 기본정보 조회 서비스**
3. 승인 후 서비스 키를 `.env`의 `EGEN_SERVICE_KEY`에 입력 (로컬 환경)
4. **GitHub Actions 배포/크롤러 환경:** 저장소의 `Settings > Secrets and variables > Actions`에 `EGEN_SERVICE_KEY` 시크릿을 등록해야 무인 자동 모니터링 크롤러가 동작합니다.

...
function safeErrorSummary(error: unknown): { code?: string; message: string } {
  if (axios.isAxiosError(error)) {
    return {
      code: error.code,
      message: error.message || 'E-Gen request failed',
    };
  }
...
async function runCrawler() {
  ...
  const TEST_LIMIT = process.env.CRAWLER_TEST_LIMIT
    ? parseInt(process.env.CRAWLER_TEST_LIMIT, 10)
    : 5;
  if (TEST_LIMIT > 0 && targetHospitals.length > TEST_LIMIT) {
    console.log(
      `⚠️ 테스트 모드: ${TEST_LIMIT}개의 병원만 처리합니다. (제한 해제는 .env에서 CRAWLER_TEST_LIMIT=0 설정)`
    );
  ...
  const { data: existingData, error: existingDataError } = await supabase
    ...
    .select('hpid, last_updated_at');

  if (existingDataError) {
    console.error('❌ Supabase 병원 특화 데이터 조회 실패:', {
  ...
  for (const hospital of targetHospitals) {
    ...
    try {
      ...
      if (!textData) {
        console.warn(
          '⚠️ 수집된 데이터가 없습니다. DB에 빈 값으로 기록하여 30일간 재검색을 방지합니다(Negative Cache).'
        );
      ...
      if (analysisResult.specialties.length === 0) {
        console.log(
          '⚠️ 추출된 전문 분야가 없습니다. DB에 빈 값으로 기록하여 30일간 재검색을 방지합니다.'
        );
        await updateHospitalSpecialties(supabase, {
          hpid: hospital.hpid,
          hospital_name: hospital.name,
